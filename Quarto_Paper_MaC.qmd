...
Table 1 presents a parsimonious model that examines the relationship between understanding and political orientations, and interpersonal confidence.

**Table 1** Logistic Regression for Interpersonal Confidence

```{r echo=FALSE, message=FALSE, warning=FALSE}
model0 <- glm(intercon ~ under + ses + sex + age_num, data = merged_data, family = binomial)
summary(model0)

model00 <- glm(intercon ~ strate + ses + sex + age_num, data = merged_data, family = binomial)
summary(model00)
...
**Figure 5** Coefplot for Interpersonal Confidence
```{r echo=FALSE, results='hide', message=FALSE, warning=FALSE}
# Assuming coef_df is already prepared as per previous instructions

# Apply the function to both models
coef_df0 <- get_coef(model0, "Model 0")
coef_df00 <- get_coef(model00, "Model 00")


# Combine the data frames and remove the intercept
coef_dfprev <- rbind(coef_df0, coef_df00)
coef_dfprev <- coef_dfprev[!coef_dfprev$Term %in% c("(Intercept)", "sex", "ses", "age_num"), ]


# Calculate 95% confidence intervals and determine significance
coef_dfprev <- coef_dfprev %>%
  mutate(
...
ggplot(coef_dfprev, aes(x = Term, y = Estimate, ymin = Lower_CI, ymax = Upper_CI, color = Model, group = Model)) +
...
Tables 2, 3 and Figure 6 show the models for understanding and political orientations.


**Table 2** OLS Regression for Understanding Orientation.

```{r echo=FALSE, message=FALSE, warning=FALSE}
model1 <- lm(under ~ ses + sex + age_num + media + digital + social + interest + extef + intef + ope, data = merged_data)
summary(model1)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
model2 <- lm(strate ~ ses + sex + age_num + media + digital + social + interest + extef + intef + ope, data = merged_data)
summary(model2)
```
